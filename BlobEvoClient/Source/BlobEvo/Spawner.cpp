...
void USpawner::GenerateRandomSpawningLocation(FVector& OutLocation) {
	...
	do {
		...
		for (int32 i = 0; i < ExistingBlobLocations.Num(); i ++) {
			if (FVector::Distance(OutLocation, ExistingBlobLocations[i]) < SpawnSpacing) {
...
void USpawner::SpawnInitialBlobCount() {
	...
	if (BlobClass) {
		...
		if (world) {
			for (int32 i = 0; i < InitialNumberOfBlobs; i++) {
				...
				ExistingBlobLocations.Add(NewBlob->GetTransform().GetTranslation());
			}
		} 
	}
}
